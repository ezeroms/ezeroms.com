...
/** Creative 一覧: involvement / Chooning 専用は除外（公開 Creative と同じ） */
function isCreativeWork(row: Record<string, unknown>): boolean {
  const normalized = normalizeWorkRow(row);
  if (normalized.work_kind === "involvement") return false;
  if (normalized.product_key === "chooning") return false;
  return true;
}
...
export default async function AdminCreativeListPage() {
  ...
  if (hasSupabaseConfig()) {
    ...
    if (error) {
      ...
      console.error("[admin/creative]", error.message);
    } else {
