-- Clips: create table (with OGP columns) — run in Supabase SQL Editor
-- Requires: public.content_status enum, public.set_updated_at()
-- (from 20260716120000_init_content_schema.sql)
...
-- If an older clip table already exists without OGP columns:
...
comment on column public.clip.og_image is 'Cached og:image (or twitter:image) URL for Pinterest-style cards';
comment on column public.clip.og_description is 'Cached og:description when available';
